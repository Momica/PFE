function correct_rate = Fun_ESRC_Multi(Train_M_right_eye,Train_M_left_eye,Train_M_mouse,Train_M_nose,train_label,Test_M_right_eye,Test_M_left_eye,Test_M_mouse,Test_M_nose,test_label,lambda)
...
for ti = 1:size(Test_M_right_eye,2)
    ...
    gap = (0.67)*gap_right_eye + (0.60)*gap_left_eye + (0.55)*gap_mouse + (0.53)*gap_nose; % ATTENTION peut être il faudrait pondéré par la confiance que l'on peut accorder à chaque features ( les eyes ont eu de meilleurs résulats par exemple).
    %keyboard;
