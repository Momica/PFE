...
while tLine~=-1
    ...
    personDestDir = strcat('../DataRetrieved/session1/3stripes/',personID); %sprintf('%03d',fix((idImage-1)/20)+1));
    disp(personDestDir);
...
%     end
    ...
    picLandmark=str2double(parts(7:end));  % ATTENTION, ici cela dépend de comment est organisé notre ficher landmarksFaces.txt
    ...
    for i = 1:landmarknum
        ...
        if i == 1
            lengthWantedForFeature = 112;
            lengthBetweenEyes = npicLandmark((2*(i+1))-1) - npicLandmark((2*i)-1);
            extraSpace = (lengthWantedForFeature-lengthBetweenEyes)/2;
            imfea = imcrop(npic,[npicLandmark(2*i-1)-extraSpace npicLandmark(2*i)-25 lengthWantedForFeature 50]);
            sizeFea = size(imfea);
        ...
        if i == 2
            imfea = imcrop(npic,[npicLandmark(2*(i+1)-1)-25 npicLandmark(2*(i+1))-28 50 52]);
            sizeFea = size(imfea);
            imFeaMatrix = reshape(imfea,sizeFea(1)*sizeFea(2),1);
        ...
        if i == 3
            ...
            lengthMouse = npicLandmark((2*(i+2))-1) - npicLandmark((2*(i+1))-1);
...
%             imfea = imcrop(npic,[(npicLandmark(2*i-1))-15 (npicLandmark(2*i))-15 lengthMouse+30 heightMouse+30]); % ATTENTION ici il faudra utiliser un autre cropping
            imfea = imcrop(npic,[(npicLandmark(2*(i+1)-1))-extraSpace (npicLandmark(2*(i+1)))-20 lengthWantedForFeature 40]); % ATTENTION i+1 car on saute l'oeil gauche dans 3 stripes
            sizeFea = size(imfea);
            imFeaMatrix = reshape(imfea,sizeFea(1)*sizeFea(2),1);
