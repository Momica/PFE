...
for id = 1:length(iddir) % process the image one by one
    ...
    personNumber =  sprintf('%03d',fix((id-1)/20)+1);
    ...
    if isempty(points)
        ...
        foundStatus = '0';
        fprintf(fw,'image %d person %s found %s\n',id,personNumber,foundStatus);
    else 
...
%             sArr=regexp(tLine,' ','split');
            ...
            fprintf(fw,'image %d person %s found %s %f %f %f %f %f %f %f %f %f %f\n',id,personNumber,foundStatus, points(1,i),points(2,i),points(3,i),points(4,i),points(5,i),points(6,i),points(7,i),points(8,i),points(9,i),points(10,i));
        end
%         fclose(fb);
%         fclose(fr);
    end 
    
    % UNCOMMENT if you want to display the images with key point 
...
% UNCOMMENT IF YOU NEED FACE DETECT AND REPLACE 1,81,1,99 below by the
% df(i) coresponding 
...
fprintf(bboxfid,'%s %d %d %d %d',[imn,imtype],1,81,1,99);
fclose(bboxfid);
