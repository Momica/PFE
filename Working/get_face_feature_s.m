function [feature] = get_face_feature_s(im,landmark,mapping,mode,patchMesh,patchSize,olp)%,FeaExtrFun
...
[imh, imw] = size(im);
% comMinX = min(min(landmark(1)-(patchMesh(2)-parity(2))/2*patchSize-(patchSize-1)/2,landmark(7)-(patchMesh(2)-parity(2))/2*patchSize-(patchSize-1)/2),...
%     landmark(5)-(patchMesh(2)-parity(2))/2*patchSize-(patchSize-1)/2)-3-olp;
% comMaxX = max(max(landmark(3)+(patchMesh(2)-parity(2))/2*patchSize+(patchSize-1)/2,landmark(9)+(patchMesh(2)-parity(2))/2*patchSize+(patchSize-1)/2),...
%     landmark(5)+(patchMesh(2)-parity(2))/2*patchSize+(patchSize-1)/2)+3+olp;
% comMinY = min(landmark(2)-(patchMesh(1)+parity(1))/2*patchSize-(patchSize-1)/2,landmark(4)-(patchMesh(1)+parity(1))/2*patchSize-(patchSize-1)/2)-3-olp;
% comMaxY = max(landmark(8)+(patchMesh(1)-parity(1))/2*patchSize+(patchSize-1)/2,landmark(10)+(patchMesh(1)-parity(1))/2*patchSize+(patchSize-1)/2)+3+olp;
...
comMinX = minlandmarkx-(patchMesh(2)-parity(2))/2*patchSize-(patchSize-1)/2-3-olp;
comMaxX = maxlandmarkx+(patchMesh(2)-parity(2))/2*patchSize+(patchSize-1)/2+3+olp;
comMinY = minlandmarky-(patchMesh(1)+parity(1))/2*patchSize-(patchSize-1)/2-3-olp;
comMaxY = maxlandmarky+(patchMesh(1)-parity(1))/2*patchSize+(patchSize-1)/2+3+olp;
...
% figure,imshow(im,[]);
for i = 1:landmarknum
     startxMesh = landmark(2*i-1)-(patchMesh(2)-parity(2))/2*patchSize-parity(2)*(patchSize-1)/2;
     endxMesh = landmark(2*i-1)+(patchMesh(2)-parity(2))/2*patchSize+parity(2)*(patchSize-1)/2+parity(2)-1;     
     if patchMesh(1) == 7
         ...
         startyMesh = landmark(2*i)-(patchMesh(1)-parity(1))/2*patchSize-parity(1)*(patchSize-1)/2;
         endyMesh = landmark(2*i)+(patchMesh(1)-parity(1))/2*patchSize+parity(1)*(patchSize-1)/2+parity(1)-1;
     end
     ...
     for ii=1:onePatchNum;
         tim = im(gridY(ii)-olp:gridYend(ii)+olp,gridX(ii)-olp:gridXend(ii)+olp);
