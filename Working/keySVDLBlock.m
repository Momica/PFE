...
if exist(savingName) == 2
    ...
    if exist(dirBlock) == 2
        ...
        tt_dat = tem_dat;
        sizeDat = size(tt_dat);
        dirDestName = strcat('block_occlusion/',int2str(100*block_l),'/');
...
correct_rate = Fun_ESRC_gaps_block(dicos,tt_dat,ttls,parameters.lambda);
%fid = fopen(['result/demo_result_' par.nameDatabase '.txt'],'a');
fid = fopen(['result/demo_result_block.txt'],'a');
fprintf(fid,'\n%s\n','========================================');
for n = 1:length(testFeatures)
